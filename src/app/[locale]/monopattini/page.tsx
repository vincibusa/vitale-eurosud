import { getVehiclesByCategory, getVehicleSubcategory } from '@/lib/vehicles'
...
export default async function MonopattiniPage({
	...
	const vehiclesData = await getVehiclesByCategory('monopattini')
	...
	const filterSections = [
		{
			...
			options: Object.entries(subcategoryCounts).map(([label, count]) => ({
				label: label === 'urbano' ? 'Urbano' : label === 'performance' ? 'Performance' : label,
				value: label,
				count
			}))
		},
